...
public:
  ...
  Set(const Set& alt):capacity(alt.capacity), size(alt.size){
    data = new int[capacity];
    for (int i = 0; i<capacity; i++){
      data[i] = alt.data[i];
    }
  }
  // (g) [4 points] Overload the assignment operator. Same requirements as
  // copy constructor.
